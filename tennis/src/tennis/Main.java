 ...
 * @throws SQLException 
  ...
  public static void seeReservationMenu() throws SQLException {
    try {
    	conn = DriverManager.getConnection(DB_URL, USER, PASS);
        String selectUserReservation = "SELECT * FROM Reservation JOIN payment ON Reservation.paymentId = payment.paymentId WHERE username = ?";
